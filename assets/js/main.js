...
$(document).ready(function() {
    // ProArc hover enhancement
    $('.proarc-products-menu').hover(
        function() { $(this).addClass('proarc-hover'); },
        function() { $(this).removeClass('proarc-hover'); }
    );
});
...
// Wrap everything in a safety check
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (typeof jsVectorMap !== 'undefined' && mapElement) {
        map = new jsVectorMap({
            selector: '#world-map',
            map: 'world',
            markers: [
                { name: 'India', coords: [20.5937, 78.9629], id: 'india' },
                { name: 'UK', coords: [55.3781, -3.4360], id: 'uk' },
                { name: 'Germany', coords: [51.1657, 10.4515], id: 'germany' },
                { name: 'Dubai', coords: [25.2048, 55.2708], id: 'dubai' }
            ],
            markerStyle: {
                initial: { fill: '#EA5501', r: 7 },
    ...
    // 2. Global function for your List clicks
    window.focusLocation = function(countryName, address) {
        ...
        if (tooltip) {
            tooltip.innerHTML = `<strong>${countryName.toUpperCase()}</strong><br>${address}`;
            tooltip.style.display = 'block';
            
            // Position it centered on the map for jsVectorMap compatibility
    ...
    // 3. Initialize Certificate Slider
    if (typeof Swiper !== 'undefined' && document.querySelector('.rs-certificate-slider')) {
        new Swiper('.rs-certificate-slider', {
            slidesPerView: 2,
            spaceBetween: 30,
            loop: true,
            autoplay: { delay: 2500 },
...
document.addEventListener("DOMContentLoaded", function() {
    // 1. SAFE CHECK FOR MAP: Stops the "classList" and "Not Defined" errors
    ...
    // 2. FORCE SWIPER START: If your theme's auto-init fails, this forces the slider
    if (typeof Swiper !== 'undefined' && document.querySelector('.rs-certificate-slider')) {
        new Swiper('.rs-certificate-slider', {
            slidesPerView: 2,
            spaceBetween: 30,
    ...
    // 3. INITIALIZE PREVIEW: Magnific Popup for images
    if (jQuery().magnificPopup) {
        $('.rs-certificate-slider').magnificPopup({
            delegate: '.rs-popup-image',
            type: 'image',
